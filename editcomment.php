<?php
	...
	if(isset($_POST['liked'])) {
		...
		$q = "INSERT INTO `notifications` (receiver, title, c_id, sender) VALUES ((SELECT username FROM `registration` WHERE username=(SELECT username FROM `comment` WHERE id='$id')), 'comment likes', '$id', '$uname')";
		mysqli_query($dbc, $q);
		echo $nums;
		exit();
	}
	...
	if(isset($_POST['disliked'])) {
		...
		$q = "INSERT INTO `notifications` (receiver, title, c_id, sender) VALUES ((SELECT username FROM `registration` WHERE username=(SELECT username FROM `comment` WHERE id='$id')), 'comment dislikes', '$id', '$uname')";
		mysqli_query($dbc, $q);
		echo $nums;
		exit();
	}
	...
	if(isset($_POST['unliked'])) {
		...
		$q = "DELETE FROM `notifications` WHERE c_id='".$id."' AND sender='".$uname."' AND title='comment likes'" or die();
		mysqli_query($dbc, $q);
		echo $nums;
		exit();
	}
	...
	if(isset($_POST['undliked'])) {
		...
		$q = "DELETE FROM `notifications` WHERE c_id='".$id."' AND sender='".$uname."' AND title='comment dislikes'" or die();
		mysqli_query($dbc, $q);
		echo $nums;
		exit();
	}
